...
uint8_t ext2_writefile(char *fn, char *buf, uint32_t len, ext2_priv_data *priv)
{
	...
	if(inode->size == 0)
	{
		...
		{
			/* @todo */
			printf("[ext2] Sorry, can't write to files bigger than 12Kb :(\n");
			return 0;
		}
		...
		printf("[ext2] Allocated %d blocks!\n", blocks_to_alloc);
...
uint8_t ext2_touch(char *file, ext2_priv_data *priv)
{
	...
	char *fil = (char *)kmalloc(strlen((const char*)file) + 1);
	memcpy(fil, file, strlen((const char*)file) + 1);
	...
	size_t n = strexplode(fil, '/') - 1;
	n--;
	while(n)
	{
		fil += strlen((const char*)fil) + 1;
		n--;
	}
	printf("[ext2] filename: %s\n", fil);
	ext2_dir *entry = (ext2_dir *)kmalloc(sizeof(ext2_dir) + strlen((const char*)fil) + 1);
	entry->size = sizeof(ext2_dir) + strlen((const char*)fil) + 1;
	entry->namelength = strlen((const char*)fil) + 1;
	entry->reserved = 0;
	memcpy(&entry->reserved + 1, fil, strlen((const char*)fil) + 1);
	printf("[ext2] Length of dir entry: %d + %d = %d\n", sizeof(ext2_dir), strlen((const char*)fil) + 1, entry->size);
	...
	printf("[ext2] This inode is located on block %d with ioff %d\n", block, ioff);
	 ...
	 */
	char *f = (char *)kmalloc(strlen((const char*)file) + 1);
	memcpy(f, file, strlen((const char*)file) + 1);
	str_backspace(f, '/');
	...
	for(int i = 0; i < 12; i++)
	{
		...
		while(d->inode != 0) {
			...
			printf("[ext2] truesize has modulo 4 of %d, adding %d\n", truesize % 4, 4 - truesize%4);
			truesize += 4 - truesize % 4;
			uint32_t origsize = d->size;
			printf("[ext2] truesize: %d origsize: %d\n", truesize, origsize);
			if(truesize != d->size)
			{
				 ...
				 */
				d->size = truesize;
				printf("[ext2] Adjusted entry len:%d, name %s!\n", d->size, &d->reserved + 1);
			...
			printf("[ext2] skipped len: %d, name:%s!\n", d->size, &d->reserved + 1);
			passed += d->size;
			d = (ext2_dir *)((uint32_t)d + d->size);
		}
		...
		{
			printf("[ext2] Couldn't find it in DBP %d (%d > %d)\n", i, passed, priv->blocksize);
			continue;
		}
	...
	dir_write:
		...
		printf("[ext2] Wrote to %d\n", inode->dbp[i]);
		return 1;
	}
	printf("[ext2] Couldn't write.\n");
	return 0;
}
...
void ext2_write_inode(inode_t *inode_buf, uint32_t ii, ext2_priv_data *priv)
{
	...
	ext2_read_block(block_buf, priv->first_bgd, priv);
	block_group_desc_t *bgd = (block_group_desc_t*)block_buf;
	printf("[ext2] We seek BG %d\n", bg);
	/* Seek to the BG's desc */
	for(i = 0; i < bg; i++)
		bgd++;
	/* Find the index and seek to the inode */
	uint32_t index = (ii - 1) % priv->sb.inodes_in_blockgroup;
	printf("[ext2] Index of our inode is %d\n", index);
	uint32_t block = (index * sizeof(inode_t))/ priv->blocksize;
	printf("[ext2] Relative: %d, Absolute: %d\n", block, bgd->block_of_inode_table + block);
...
uint8_t ext2_read_root_directory(char *filename, ext2_priv_data *priv)
{
	...
	{
		printf("[ext2] FATAL: Root directory is not a directory!\n");
		return 0;
	}
...
{
	char *filename = (char*)kmalloc(strlen((const char*)ff) + 1);
	memcpy(filename, ff, strlen((const char*)ff) +1);
	size_t n = strexplode(filename, '/');
		...
		while(n--)
		{
			...
			for(int i = 0; i < 12; i++)
			{
				...
				if(!rc)
				{
					...
					printf("[ext2] File (%s (0x%x)) not found!\n", filename, filename);
					kfree(filename);
					return 0;
				} else {
			...
			fix:;
			uint32_t s = strlen((const char*)filename);
			filename += s + 1;
		}
...
uint8_t ext2_read_file(char *fn, char *buffer, ext2_priv_data *priv)
{
	...
	{
		printf("[ext2] File inode not found.\n");
		return 0;
	}
	for(int i = 0; i < 12; i++)
	{
		...
		if(b == 0) { printf("EOF\n"); return 1; }
		if(b > priv->sb.blocks) printf("[ext2] %s: block %d outside range (max: %d)!\n", __func__, b, priv->sb.blocks);
		printf("[ext2] Reading block: %d\n", b);
...
void ext2_init() {
    ...
    strcpy(current_dir, _temp);

    printf("[ext2] Attempt load ext2 fs on main dev.\n");
    // read superblock
	...
	{
		printf("[ext2] Invalid EXT2 signature (0x%x)!\n", sb->ext2_sig);
        printf("[ext2] Not using EXT2.\n");
        ext2_online = false;
        return;
	}
    ...
    printf("[ext2] Size of volume: %d bytes\n", volume_size);
    ...
    printf("[ext2] There are %d block group(s).\n", number_of_bgs0);
    ...
    _priv = priv;
     ...
     if (ext2_read_root_directory((char*)1, priv)) {
		 printf("[ext2] Root directory read successful, reading enabled.\n");
	 }

     char* c = (char*)kmalloc(4);
     strcpy(c, "bruh");
     //ext2_writefile("/ABC123test.txt", c, 4, priv);
	 printf("[ext2] WARNING: File writing is broken, so attempting to create a new file will not work. Sorry :/\n");
}
...
bool CMD_cat(int argc, char** argv) {
    ...
    if (strcmp(current_dir, "/") != 0) {
        char* ndir;
        memcpy(to_open, current_dir, strlen((const char*)current_dir));
        size_t i = strlen((const char*)current_dir);
        to_open[i] = '/';
        i++;
        for (; i < strlen((const char*)current_dir) + strlen((const char*)argv[1]) + 1; i++) {
            to_open[i] = argv[1][i - (strlen((const char*)current_dir) + 1)];
        }
    } else {
        char* ndir;
        memcpy(to_open, current_dir, strlen((const char*)current_dir));
        size_t i = strlen((const char*)current_dir);
        for (; i < strlen((const char*)current_dir) + strlen((const char*)argv[1]); i++) {
            to_open[i] = argv[1][i - (strlen((const char*)current_dir))];
        }
    }
